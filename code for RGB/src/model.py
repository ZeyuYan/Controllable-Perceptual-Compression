...
class Model(nn.Module):
    ...
    def __init__(self, args, logger, storage_train=defaultdict(list), storage_test=defaultdict(list), model_mode=ModelModes.TRAINING, 
        ...
        self.image_dims = (3,256,256)#self.args.image_dims  # Assign from dataloader
    ...
    def compression_forward(self, x, freezE=False):
        """
        ...
        #latents_quantized = latents_quantized.detach()
    ...
    def distortion_loss(self, x_gen, x_real):
        ...
        sq_err = torch.sqrt(torch.sum((x_gen - x_real)**2, dim=[1,2,3]))
    ...
    def compression_loss(self, intermediates, hyperinfo, reconstruct=None):
        
        x_real = intermediates.input_image
        x_gen = intermediates.reconstruction
        if reconstruct != None:
            x_real = reconstruct.input_image
        ...
        weighted_R_D_loss = self.args.k_R * weighted_rate + weighted_distortion
    ...
    def GAN_loss(self, intermediates, train_generator=False):
        ...
        # Bookkeeping 
        if (self.step_counter % self.log_interval == 1):
            self.store_loss('D_gen', torch.mean(disc_out.D_gen_logits).item())
            self.store_loss('D_real', torch.mean(disc_out.D_real_logits).item())
    ...
    def forward(self, x, train_generator=False, return_intermediates=False, writeout=True, reconstruct=None, fixE=False):
        ...
        intermediates, hyperinfo = self.compression_forward(x, freezE=fixE)
        ...
        compression_model_loss = self.compression_loss(intermediates, hyperinfo, reconstruct)
