...
def update_dlr(args, optimizer, itr, logger):
    lr = get_scheduled_params(args.learning_rate*10, args.lr_schedule, itr)
    for param_group in optimizer.param_groups:
        old_lr = param_group['lr']
        if old_lr != lr:
            logger.info('=============================')
...
def save_model(model, optimizers, mean_epoch_loss, epoch, device, args, logger, multigpu=False):
    ...
    model_path = os.path.join(directory, '{}_epoch{}_idx{}.pt'.format(model_name, epoch, model.step_counter))
