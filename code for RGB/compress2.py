...
def compress_and_decompress(args):
    ...
    gener = torch.load(args.decoder)["model"]
    gener.eval()
    ...
    with torch.no_grad():
        ...
        for idx, (data, bpp, filenames) in enumerate(tqdm(eval_loader), 0):
            ...
            if args.reconstruct is True:
                ...
                final_out = torch.zeros(data.size())
                hific_out = torch.zeros(data.size())
                hight = data.size(2)
                width = data.size(3)
                num_h = int((hight-1)/crop_batch) + 1
                num_w = int((width-1)/crop_batch) + 1
                for i_h in range(num_h):
                    start_h = i_h*crop_batch
                    end_h = min(start_h + crop_batch, hight)
                    for i_w in range(num_w):
                        start_w = i_w*crop_batch
                        end_w = min(start_w + crop_batch, width)

                        patch_data = data[:,:,start_h:end_h,start_w:end_w]
                        reconstruction,_ = model.compression_forward(patch_data)
                        q_bpp = reconstruction.q_bpp
                        reconstruction_mse = reconstruction.reconstruction
                        latent = reconstruction.latents_quantized
                        rec_dim = reconstruction_mse.size()

                        reconstruction = gener(latent)
                        reconstruction = reconstruction[:, :, :rec_dim[2], :rec_dim[3]]

                        final_out[:,:,start_h:end_h,start_w:end_w]=reconstruction
                        hific_out[:,:,start_h:end_h,start_w:end_w]=reconstruction_mse
                reconstruction = final_out
                reconstruction_hific = hific_out
            ...
            for subidx in range(reconstruction.shape[0]):
                ...
                fname = os.path.join(args.output_dir, "{}.png".format(filenames[subidx]))
                fname_hific = os.path.join(args.output_dir, "{}_mse.png".format(filenames[subidx]))
                torchvision.utils.save_image(reconstruction[subidx], fname, normalize=False)
                #torchvision.utils.save_image(reconstruction_hific[subidx], fname_hific, normalize=False)
            ...
            LPIPS_total[n:n + B] = 0 #perceptual_loss.data
            n += B
...
def main(**kwargs):
    ...
    parser.add_argument("--decoder", default="./model/E100_ours_0-01low.pth", type=str, help="decoder path")
