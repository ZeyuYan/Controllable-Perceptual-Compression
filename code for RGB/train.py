...
from srresnet_depU import _NetG,_NetD, _NetG_bn
        ...
        for idx, (data, bpp) in enumerate(tqdm(train_loader, desc='Train'), 0):
            ...
            try:
                if model.use_discriminator is True:
                    ...
                    target = reconstruction.input_image
                    input = reconstruction.reconstruction
                    iteration = iteration + 1

                    discr.zero_grad()

                    D_result = discr(target, input).squeeze()
                    D_real_loss = -D_result.mean()

                    z_ = torch.randn((input.size()[0], 100)).view(-1, 100, 1, 1)
                    z_ = Variable(z_.cuda())

                    G_result = gener(input, z_)
                    D_result = discr(G_result.data, input).squeeze()
                    ...
                    interpolated2 = Variable(input, requires_grad=True)
                    ...
                    grad_l2norm = torch.sqrt(torch.sum(grad ** 2, dim=1)) - 1
                    ...
                    G_result = gener(input, z_)
                    D_result = discr(G_result, input).squeeze()

                    #mse_low = torch.mean((dsmpl(G_result) - dsmpl(target))**2)

                    mse_loss = torch.mean((G_result - target)**2)
                    move_loss = torch.sqrt(torch.sum((G_result - input)**2, dim=[1,2,3]))
                    mse.append(mse_loss.data)

                    G_train_loss = - D_result.mean() + 0.1 * move_loss.mean()
